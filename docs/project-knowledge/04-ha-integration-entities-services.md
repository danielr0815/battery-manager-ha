...
Seit v0.29.0 enthält jeder `cascades[]`-Eintrag `source_name`, `member_details`,
`terminal_name` und eine per-Slot `schedule`. Jeder Block trägt `start`, `end`,
`root_input_wh`, `terminal_energy_wh`, `sources` und `activities`; letztere
...
| `threshold_merge_margin_wh` | Gestresster Export der ausgewählten zusammenhängenden Vollakku-Episode; steuert die 0–250-Wh-Terminal-Credit-Rampe und bleibt auch bei `threshold_horizon_end = null` sichtbar |
...
| `BatteryManagerFeedInSwitch` / `early_feed_in` (v0.23.0) | Laufzeit-Ein/Aus der vorzeitigen Netzeinspeisung (F-FEEDIN R8). **An** (Default) = der Executor treibt den AC-Setpoint per Plan inkl. Trim; **Aus** = Pause für automatische Planung im gesamten Horizont und Ausführung; schreibt sofort einmalig 0 (nur Auto-Modus) und berechnet SOC-Kurve/Tageswerte neu. Natürlicher Export bleibt sichtbar. Manuelle Setpoints werden nur für heute abgebildet; Folgetage bleiben pausiert (v0.38.0). Store-persistiert, immer verfügbar; existiert nur bei aktivem Options-Toggle `feedin_enabled`. Geht über den einen Einstiegspunkt `coordinator.async_set_feedin_enabled` |
