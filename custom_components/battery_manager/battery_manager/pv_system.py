...
class PVSystem:
    ...
    def calculate_hourly_production_wh(
        ...
        reference_start = reference_datetime.replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        target_start = target_datetime.replace(
            hour=0, minute=0, second=0, microsecond=0
        )
    ...
    def _calculate_hourly_pv(
        self, daily_forecasts: List[float], day_offset: int, hour: int
    ) -> float:
        ...
        # Afternoon production
            ...
            return (
                afternoon_energy_kwh * 1000
            ) / afternoon_hours  # Convert to Wh per hour
    ...
    def get_daily_production_estimate(
        self, daily_forecasts: List[float], day_offset: int
    ) -> float:
