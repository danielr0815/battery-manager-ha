...
class BatteryManagerSimulator:
    ...
    def _validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if "battery_capacity_wh" in config:
            if config["battery_capacity_wh"] <= 0:
                raise ValueError(f"Battery capacity must be positive, got {config['battery_capacity_wh']}")
        
        if "battery_soc_min_percent" in config:
            min_soc = config["battery_soc_min_percent"]
            if not 0 <= min_soc <= 100:
                raise ValueError(f"Battery min SOC must be between 0-100%, got {min_soc}")
        
        if "battery_soc_max_percent" in config:
            max_soc = config["battery_soc_max_percent"]
            if not 0 <= max_soc <= 100:
                raise ValueError(f"Battery max SOC must be between 0-100%, got {max_soc}")
        
        # Check min <= max SOC
        if ("battery_soc_min_percent" in config and 
            "battery_soc_max_percent" in config):
            min_soc = config["battery_soc_min_percent"]
            max_soc = config["battery_soc_max_percent"]
            if min_soc >= max_soc:
                raise ValueError(f"Battery min SOC ({min_soc}%) must be less than max SOC ({max_soc}%)")
        ...
        # Power validation
        power_params = [
            "pv_peak_power_w", "ac_base_load_w", "dc_base_load_w"
        ]
        ...
        if "controller_max_threshold_percent" in config:
            ...
            if not 0 <= threshold <= 100:
                raise ValueError(f"Controller max threshold must be between 0-100%, got {threshold}")
    ...
    def _validate_inputs(self, current_soc_percent: float, daily_forecasts: List[float]) -> None:
        ...
        if not 0 <= current_soc_percent <= 100:
            raise ValueError(f"SOC must be between 0 and 100%, got {current_soc_percent}")
